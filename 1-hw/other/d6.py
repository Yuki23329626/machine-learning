...
def linear_regression ( x_train, y_train, x_val, y_val, trn_s, val_s):
  wln = np.linalg.inv(x_train.T.dot( x_train)).dot(x_train.T.dot(y_train))
  return  wln

def ridge_regression ( x_train, y_train, x_val, y_val, trn_s, val_s, lamda):
  wln = np.linalg.inv(x_train.T.dot( x_train)).dot(x_train.T.dot(y_train)+lamda*np.eye(x_train[0]))
  return  wln

def calculate_error_L1 ( x_train, y_train, x_val, y_val, trn_s, val_s):
    wln = linear_regression( x_train, y_train, x_val, y_val, trn_s, val_s)
    trn_error = abs((x_train.dot(wln) - y_train).sum()/trn_s)
    val_error = abs((x_val.dot(wln) - y_val).sum()/val_s)
    return  wln, trn_error, val_error

def calculate_error_L2 ( x_train, y_train, x_val, y_val, trn_s, val_s):
    wln = ridge_regression( x_train, y_train, x_val, y_val, trn_s, val_s)
    trn_error = abs((x_train.dot(wln) - y_train).sum()/trn_s)
    val_error = abs((x_val.dot(wln) - y_val).sum()/val_s)
    return  wln, trn_error, val_error
...
def leave_one_out(x ,y ):
    ...
    for train_index, test_index in leaveOneOut.split(x) : 
        ...
        leave_one_out += calculate_error_L1(X_train, Y_train, X_test, Y_test, 14, 1)[2]
...
def five_fold(x, y):
    ...
    for train_index , test_index in kf.split( x_train ):
        ...
        X_train, X_test = x[train_index], x[test_index]
        Y_train, Y_test = y[train_index], y[test_index]
        five_fold_error += calculate_error_L1(X_train, Y_train, X_test, Y_test,12, 3)[2]
...
#### from function 
wln, train_error, test_error = calculate_error_L1 (x_train, y_train, x_test, y_test, 15, 5)
...
x5_wln, x5_train_error, x5_test_error = calculate_error_L1 (x5_train, y_train, x5_test, y_test, 15, 5)
print ("\n\n")
print ("Degree 5")
...
x10_wln, x10_train_error, x10_test_error = calculate_error_L1 (x10_train, y_train, x10_test, y_test, 15, 5)
print ("\n\n")
print ("Degree 10")
...
x14_wln, x14_train_error, x14_test_error = calculate_error_L1 (x14_train, y_train, x14_test, y_test, 15, 5)
print ("\n\n")
print ("Degree 14")

...
def ridge_regression ( x_training, y_training, x_valid, y_valid, sum_training, sum_valid, lamda):
  wlin = np.linalg.inv(x_training.T.dot( x_training)+lamda*np.eye(x_training.shape[0])).dot(x_training.T.dot(y_training))
  return  wlin
...
wlin_degree14, error_training_degree14, error_testing_degree14 = calculate_error_L2 (x_training_degree14, y_training, x_testing_degree14, y_test, x_training.shape[0], x_testing.shape[0], lamda)
print ("\n--Degree 14--")
...
y_degree_m4=0
for i in range (0, 15):
    y_degree_m4 += x_line**(14-i)* wlin_degree14 [i]
